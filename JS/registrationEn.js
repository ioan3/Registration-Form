...
function onPasswordTextChanged() {
    if(!psw.value) {
        ...
        pswMessage.innerHTML = 'Field should not be empty';
    }
    else if (psw.value.length < 6) {
        psw.style.backgroundColor = '#dc3545';
        pswMessage.style.color = '#dc3545';
        pswMessage.innerHTML = 'password must be at least 6 characters';
    }
    else {
...
function onRepeatPasswordChanged() {
    if(!repeatPsw.value) {
        ...
        pswMessage.style.color = '#dc3545'
        pswMessage.innerHTML = 'Confirmation password does not match';
    }
    else {
...
function Register() {
    window.alert('Registration finished successfully, Check your mail for verification.');
}